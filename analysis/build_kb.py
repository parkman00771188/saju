...
CONCEPTS = {
    **{s: STEM_NAMES[s] for s in STEMS},
    ...
    # 격국
    "정관격": ["정관격"], "편관격": ["편관격", "칠살격"], "정재격": ["정재격"], "편재격": ["편재격"], "식신격": ["식신격"],
    "상관격": ["상관격"], "정인격": ["정인격"], "편인격": ["편인격"], "건록격": ["건록격"], "양인격": ["양인격", "월겁격"],
    # 육친
    "아버지": ["아버지", "아빠", "부친"], "어머니": ["어머니", "엄마", "모친"], "배우자": ["배우자", "남편", "아내", "와이프", "부인"],
    "자식": ["자식", "자녀", "아이가", "아들", "딸"], "형제": ["형제", "자매", "남매"],
    # 직업군
    "공직": ["공무원", "공직", "공기업"], "사업": ["사업가", "사업을", "자영업", "장사"], "프리랜서": ["프리랜서", "1인 기업", "혼자 일"],
    "의료": ["의사", "의료", "간호", "약사", "한의"], "법조": ["변호사", "법조", "판사", "검사", "법률"], "교육": ["교사", "교수", "강사", "교육자", "선생"],
    "예술연예": ["예술", "연예", "배우", "가수", "예술가", "디자이너", "작가"], "IT기술": ["개발자", "IT", "엔지니어", "기술자", "프로그래"],
    "금융": ["금융", "은행", "회계", "세무", "증권"], "영업서비스": ["영업", "서비스업", "판매", "마케팅", "요식"], "연구": ["연구원", "연구직", "학자", "연구를"],
}
...
for s in STEMS:
    for g in TEN_GODS:
        ...
        PATTERNS["%s+%s" % (s, g)] = [alt + r"[^.?!]{0,30}" + gg, gg + r"[^.?!]{0,30}" + alt]
# 자리 × 십성 (년·월·일·시)
POS_WORDS_RX = {"년": r"(년주|년지|연주|연지|년간|연간)", "월": r"(월주|월지|월간)", "일": r"(일지|배우자\s?자리|배우자궁)", "시": r"(시주|시지|시간에)"}
for pos, prx in POS_WORDS_RX.items():
    for g in TEN_GODS:
        gg = "(%s)" % ("편관|칠살" if g == "편관" else g)
        PATTERNS["%s지+%s" % (pos, g)] = [prx + r"[^.?!]{0,12}" + gg, gg + r"[^.?!]{0,8}" + prx]
# 십성 운 (대운·세운에서 그 십성이 들어올 때)
for g in TEN_GODS:
    gg = "(%s)" % ("편관|칠살" if g == "편관" else g)
    PATTERNS["%s운" % g] = [gg + r"\s?(대운|세운|운이|운에|운으로|운은|운을|운일|년운|월운)", gg + r"이\s?(들어오|오는|왔)"]
...
CLAIMS = {
    ...
    # 직장 (추가)
    "직장 안정·장기근속": ("직장", 1, [r"안정적인 직장", r"오래 다니", r"한 직장", r"정년", r"안정된 직장"]),
    "사업 실패·부도": ("직장", -1, [r"부도", r"망하", r"사업 실패", r"폐업", r"사업이 (안|잘 안)", r"쫄딱"]),
    "자영업·장사": ("직장", 0, [r"자영업", r"장사", r"가게를", r"매장"]),
    "해외 근무·출장": ("직장", 0, [r"해외 (근무|취업|사업)", r"출장", r"외국계", r"무역"]),
    "두 가지 일·부업": ("직장", 0, [r"투잡", r"부업", r"두 가지 일", r"겸업", r"N잡"]),
    "취업이 늦음·공백": ("직장", -1, [r"취업이 (늦|안)", r"백수", r"공백", r"쉬는 (기간|시기)", r"놀고"]),
    # 금전 (추가)
    "배우자·집안 덕 재물": ("금전", 1, [r"처덕", r"남편 덕", r"아내 덕", r"배우자 덕", r"집안이 (부유|잘)", r"물려받"]),
    "유산·상속": ("금전", 0, [r"유산", r"상속", r"증여"]),
    "월급형·고정 수입": ("금전", 0, [r"월급", r"고정 수입", r"안정적인 수입", r"봉급"]),
    "사업 수입·큰 흐름": ("금전", 0, [r"사업 (수입|소득)", r"매출", r"큰 흐름", r"큰 단위"]),
    # 연애 (추가)
    "연상·연하 인연": ("연애", 0, [r"연상", r"연하", r"나이 차", r"나이차"]),
    "재혼·두 번 결혼": ("연애", 0, [r"재혼", r"두 번 결혼", r"결혼을 두 번", r"두 번째 결혼"]),
    "배우자 덕·능력": ("연애", 1, [r"배우자(가|의) (능력|덕|복)", r"능력 있는 (배우자|남편|아내)", r"좋은 배우자", r"배우자 복"]),
    "연애가 늦음·서툼": ("연애", -1, [r"연애가 (늦|서툴|어렵)", r"모태솔로", r"연애 경험이", r"연애를 못"]),
    "동거·사실혼": ("연애", 0, [r"동거", r"사실혼", r"혼전"]),
    "주도권·기 센 배우자": ("연애", 0, [r"기가 (세|센)", r"주도권", r"휘어잡", r"잡혀 산"]),
    # 건강 (추가)
    "갑상선·내분비": ("건강", -1, [r"갑상선", r"내분비", r"당뇨", r"호르몬"]),
    "치아·잇몸": ("건강", -1, [r"치아", r"잇몸", r"이가 (안|아)", r"치과"]),
    "체중·다이어트": ("건강", 0, [r"살이 (찌|빠)", r"다이어트", r"비만", r"체중"]),
    "두통·어지럼": ("건강", -1, [r"두통", r"머리가 아", r"어지럼", r"편두통"]),
    # 학업 (추가)
    ...
    "가족 부양·책임": ("가족", 0, [r"부양", r"가족을 책임", r"집안을 책임", r"가장 역할", r"장남", r"장녀"]),
}
